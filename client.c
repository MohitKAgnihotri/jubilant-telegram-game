...
void initGrid()
{
    ...
    if (grid[player1.playerPosition.x][player1.playerPosition.y] == TILE_TOMATO) {
        grid[player1.playerPosition.x][player1.playerPosition.y] = TILE_GRASS;
        player1.numTomatoes--;
    }

    // ensure grid isn't empty
    while (player1.numTomatoes == 0)
        initGrid();
}
...
void moveTo(int x, int y)
{
    ...
    if (!(abs(player1.playerPosition.x - x) == 1 && abs(player1.playerPosition.y - y) == 0) &&
        !(abs(player1.playerPosition.x - x) == 0 && abs(player1.playerPosition.y - y) == 1)) {
        fprintf(stderr, "Invalid move attempted from (%d, %d) to (%d, %d)\n", player1.playerPosition.x, player1.playerPosition.y, x, y);
        return;
    }
...
void handleKeyDown(SDL_KeyboardEvent* event)
{
    ...
    if (event->keysym.scancode == SDL_SCANCODE_UP || event->keysym.scancode == SDL_SCANCODE_W)
        moveTo(player1.playerPosition.x, player1.playerPosition.y - 1);
    ...
    if (event->keysym.scancode == SDL_SCANCODE_LEFT || event->keysym.scancode == SDL_SCANCODE_A)
        moveTo(player1.playerPosition.x - 1, player1.playerPosition.y);
...
void drawGrid(SDL_Renderer* renderer, SDL_Texture* grassTexture, SDL_Texture* tomatoTexture, SDL_Texture* playerTexture)
{
    ...
    dest.x = 64 * player1.playerPosition.x;
    dest.y = 64 * player1.playerPosition.y + HEADER_HEIGHT;
...
void drawUI(SDL_Renderer* renderer)
{
    ...
    sprintf(levelStr, "Level: %d", player1.level);
...
int main(int argc, char* argv[])
{
    ...
    player1.playerPosition.x = player1.playerPosition.y = GRIDSIZE / 2;
    initGrid();
